...
def test_coulomb_forces():
    c = coulomb(n_boxes_short_range,
                Test_L,
                p_error)
    c.compute_optimal_cutoff(p_error, Test_L, Test_Labels, neighbours[0], neighbours[1], r_switch, 0.49 * Test_L[0],
                             Test_Positions)
...
def test_SymmetriesPotC():
    ...
    potential.compute_optimal_cutoff(p_error, Test_L, Test_Labels, neighbours[0], neighbours[1], r_switch,
                                     0.49 * Test_L[0], Test_Positions)
    ...
    assert (
    abs(result[0] / result[1]) < 0.5 + 10 ** (-8)), "Potential does not have the expected symmetrie. P2 should be P1*2."
    assert (
    abs(result[2] / result[1]) < 0.5 + 10 ** (-8)), "Potential does not have the expected symmetrie. P2 should be P3*2."
    return
